...
func (bc *Blockchain) CopyTransactionPool() []*Transactions {
...
func (bc *Blockchain) ValidProof(nonce int, previousHash [32]byte, transactions []*Transactions, difficulty int) bool {
	zeros := strings.Repeat("0", difficulty)
	guessBlock := Block{0, nonce, previousHash, transactions}
	guesHashStr := fmt.Sprintf("%x", guessBlock.Hash())
	return guesHashStr[:difficulty] == zeros
}
...
func main() {
	...
	blockChain.CreateBlock(nonce, previousHash1)

	blockChain.AddTransaction("C", "D", 2.0)
	blockChain.AddTransaction("X", "Y", 3.0)
	previousHash2 := blockChain.LastBlock().Hash()
	nonce = blockChain.ProofOfWork()
	blockChain.CreateBlock(nonce, previousHash2)
